<script lang="ts">
  ...
  import { fetchMe, logout } from "$lib/services/api";
  ...
  onMount(async () => {
    try {
      ...
      userName = me?.full_name || me?.organization_name || "Account";
    } catch {
      // keep fallback
    }
  });
  ...
  <Sidebar.Header class="px-3.5 py-3 group/header">
    <div class="flex items-center gap-5">
      ...
      <div class="relative">
        <a
          href="/home"
          class="flex items-center gap-x-3 text-[15px] py-2 group-data-[collapsible=icon]:group-hover/header:opacity-0 transition-opacity duration-200"
        >
          <Orbit class="h-6 w-6" />
        </a>
        <!-- Hidden trigger that appears on hover when collapsed -->
        <Sidebar.Trigger
          class="absolute inset-0 opacity-0 group-data-[collapsible=icon]:group-hover/header:opacity-100 hover:bg-gray-100 rounded transition-all duration-200 flex items-center justify-center"
        />
      </div>
      ...
      <Sidebar.Trigger
        class="ml-auto hover:bg-gray-100 p-1 rounded transition-colors group-data-[collapsible=icon]:hidden"
      />
    </div>
  </Sidebar.Header>
            ...
            <Sidebar.Menu class="">
              <Sidebar.MenuItem class="">
                <Sidebar.MenuButton asChild tooltipContent="Home">
                  <a
                    href="/home"
                    class="flex items-center gap-x-3 text-[15px] py-2"
                  >
                    <Home class="h-5 w-5" />
                    <span>Home</span>
                  </a>
      ...
      <Sidebar.MenuItem class="">
        <DropdownMenu.Root>
          <DropdownMenu.Trigger>
            {#snippet child({ props })}
              ...
              >
                ...
                <ChevronUp
                  class="ml-auto h-5 w-5 group-data-[collapsible=icon]:hidden"
                />
              </Sidebar.MenuButton>
            {/snippet}
          </DropdownMenu.Trigger>
          <DropdownMenu.Content
            ...
            portalProps={{}}
          >
            <DropdownMenu.Item class="py-2 px-4" inset asChild>
            ...
            <DropdownMenu.Item
              class="py-2 px-4 data-highlighted:bg-red-50"
              inset
              variant="destructive"
