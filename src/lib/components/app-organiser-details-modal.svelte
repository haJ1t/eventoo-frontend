<script lang="ts">
	...
	let {
		open = $bindable(false),
		organiser = null,
	}: { open?: boolean; organiser?: any } = $props();
	...
	function submitRating() {
		if (userRating > 0 && reviewText.trim()) {
			...
			console.log(
				`User rated organiser ${organiser?.name} with ${userRating} stars and review: ${reviewText}`,
			);
		}
	}
...
<Dialog bind:open>
	<DialogContent
		portalProps={{}}
		class="max-w-4xl max-h-[90vh] overflow-y-auto p-0"
	>
		{#if organiser}
				...
				<DialogHeader class="space-y-2">
					<div class="flex items-center gap-2">
						...
						<div class="flex items-center text-amber-600 text-sm">
							<Star
								class="w-4 h-4 mr-1 fill-amber-400 text-amber-400"
							/>
							{organiser.rating} / 5
						</div>
					</div>
					<DialogTitle class="text-3xl font-bold"
						>{organiser.name}</DialogTitle
					>
					...
					<div class="space-y-3">
						...
						<div class="space-y-2">
							<div class="flex items-center gap-2">
								...
								<a
									href={`mailto:${organiser.contact.email}`}
							...
							<div class="flex items-center gap-2">
								...
								<a
									href={`tel:${organiser.contact.phone}`}
					...
					<div class="bg-gray-50 p-6 rounded-lg">
						{#if !hasRated}
							<p class="text-gray-600 mb-4">
								How would you rate this organiser?
							</p>
						...
						{:else}
							<div class="text-center">
								...
								<div
									class="flex justify-center items-center gap-2 mb-3"
								>
								...
								<div
									class="bg-white p-4 rounded-lg border mb-4"
								>
